...
if [[ "$CHOICES" == *"CORE"* ]]; then
    ...
    $INSTALL_CMD "${CORE_PKGS[@]}" && log "[✔] Core Packages: SUCCESS" || log "[✘] Core Packages: FAILED (Some packages may not be available)"
fi
...
if [[ "$CHOICES" == *"DESKTOP"* ]]; then
    if [[ "$DE" == "kde" ]]; then
        ...
        # 2. Setup Dock Taskbar pinned applications in exact order
        USER_UID=$(id -u "$REAL_USER")
        DBUS_ENV="export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/$USER_UID/bus;"
...
EOF
        su - "$REAL_USER" -c "$DBUS_ENV qdbus org.kde.plasmashell /PlasmaShell org.kde.PlasmaShell.evaluateScript \"\$(cat $PLASMA_JS)\"" || log "[✘] Failed to evaluate KDE dock script via qdbus."
        rm -f "$PLASMA_JS"
