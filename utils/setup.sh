...
if [[ "$CHOICES" == *"DESKTOP"* ]]; then
    if [[ "$DE" == "kde" ]]; then
        ...
        # Handle Plasma 6/Qt6 qdbus binary differences
        QDBUS_CMD="qdbus"
...
EOF
        su - "$REAL_USER" -c "$DBUS_ENV $QDBUS_CMD org.kde.plasmashell /PlasmaShell org.kde.PlasmaShell.evaluateScript \"\$(cat $PLASMA_JS)\"" || log "[✘] Failed to evaluate KDE dock script via $QDBUS_CMD."
        rm -f "$PLASMA_JS"
