...
if pgrep -i plasmashell >/dev/null || pgrep -i kwin_wayland >/dev/null || pgrep -i kwin_x11 >/dev/null; then
    ...
    if [[ "$COMBINED" == *"kde"* || "$COMBINED" == *"plasma"* ]]; then DE="kde"
    elif [[ "$COMBINED" == *"gnome"* ]]; then DE="gnome"
    elif [[ "$COMBINED" == *"cinnamon"* ]]; then DE="cinnamon"
    elif [[ "$COMBINED" == *"cosmic"* ]]; then DE="cosmic"
    fi
fi

# Detect Package Manager
if command -v dnf5 &>/dev/null; then
    PKG_MGR="dnf5"; INSTALL_CMD="dnf5 install -y"; REMOVE_CMD="dnf5 remove -y"; UPDATE_CMD="dnf5 update -y"
elif command -v dnf &>/dev/null; then
    PKG_MGR="dnf4"; INSTALL_CMD="dnf install -y"; REMOVE_CMD="dnf remove -y"; UPDATE_CMD="dnf update -y"
elif command -v apt-get &>/dev/null; then
    PKG_MGR="apt"; INSTALL_CMD="DEBIAN_FRONTEND=noninteractive apt-get install -y"; REMOVE_CMD="DEBIAN_FRONTEND=noninteractive apt-get remove -y"; UPDATE_CMD="apt-get update -y"
elif command -v pacman &>/dev/null; then
    PKG_MGR="pacman"; INSTALL_CMD="pacman -S --noconfirm"; REMOVE_CMD="pacman -Rns --noconfirm"; UPDATE_CMD="pacman -Sy"
elif command -v zypper &>/dev/null; then
    PKG_MGR="zypper"; INSTALL_CMD="zypper in -y"; REMOVE_CMD="zypper rm -y"; UPDATE_CMD="zypper ref"
else
...
CHOICES=$(whiptail --title "Linux Workstation Bootstrap Installer" \
    ...
    "DESKTOP" "Configure KDE Wallpaper (from Repo) and Pinned Dock" ON \
    3>&1 1>&2 2>&3)
...
if [[ "$CHOICES" == *"PREREQS"* ]]; then
    ...
    $INSTALL_CMD curl flatpak golang git wget unzip
...
EOF
        ...
        if [[ "$PKG_MGR" == "dnf5" ]]; then
            [ ! -f /etc/yum.repos.d/brave-browser.repo ] && dnf5 config-manager addrepo --from-repofile=https://brave-browser-rpm-release.s3.brave.com/brave-browser.repo && rpm --import https://brave-browser-rpm-release.s3.brave.com/brave-core.asc
            [ ! -f /etc/yum.repos.d/sublime-text.repo ] && rpm --import https://download.sublimetext.com/sublimehq-pub.gpg && dnf5 config-manager addrepo --from-repofile=https://download.sublimetext.com/rpm/stable/x86_64/sublime-text.repo
            [ ! -f /etc/yum.repos.d/tailscale.repo ] && dnf5 config-manager addrepo --from-repofile="$TS_REPO"
            log "[✔] Configured DNF5 repositories."
        else
            $INSTALL_CMD dnf-plugins-core
            [ ! -f /etc/yum.repos.d/brave-browser.repo ] && dnf config-manager --add-repo https://brave-browser-rpm-release.s3.brave.com/brave-browser.repo && rpm --import https://brave-browser-rpm-release.s3.brave.com/brave-core.asc
            [ ! -f /etc/yum.repos.d/sublime-text.repo ] && rpm --import https://download.sublimetext.com/sublimehq-pub.gpg && dnf config-manager --add-repo https://download.sublimetext.com/rpm/stable/x86_64/sublime-text.repo
            [ ! -f /etc/yum.repos.d/tailscale.repo ] && dnf config-manager --add-repo "$TS_REPO"
...
if [[ "$CHOICES" == *"CORE"* ]]; then
    ...
    [[ "$PKG_MGR" == "apt" ]] && BRAVE_PKG="brave-browser" || BRAVE_PKG="brave-origin"
    ...
    $INSTALL_CMD "${CORE_PKGS[@]}" && log "[✔] Core Packages: SUCCESS" || log "[✘] Core Packages: FAILED"
fi
...
if [[ "$CHOICES" == *"DEBLOAT"* ]]; then
    ...
    VALID_REMOVE=()
    for pkg in "${BLOAT[@]}"; do
        if [[ "$PKG_MGR" =~ ^(dnf4|dnf5|zypper)$ ]] && rpm -q "$pkg" &>/dev/null; then VALID_REMOVE+=("$pkg")
        elif [[ "$PKG_MGR" == "apt" ]] && dpkg -l "$pkg" &>/dev/null; then VALID_REMOVE+=("$pkg")
        elif [[ "$PKG_MGR" == "pacman" ]] && pacman -Qq "$pkg" &>/dev/null; then VALID_REMOVE+=("$pkg")
        fi
    done

    if [ ${#VALID_REMOVE[@]} -gt 0 ]; then
        log "[*] Removing (${#VALID_REMOVE[@]}) confirmed installed packages: ${VALID_REMOVE[*]}"
        $REMOVE_CMD "${VALID_REMOVE[@]}"
    else
        log "[=] No matching bloatware packages found."
    fi

    [[ "$PKG_MGR" == "dnf5" ]] && (dnf5 autoremove -y && dnf5 clean all)
    [[ "$PKG_MGR" == "dnf4" ]] && (dnf autoremove -y && dnf clean all)
    [[ "$PKG_MGR" == "apt" ]] && (apt-get autoremove -y && apt-get clean)
    [[ "$PKG_MGR" == "pacman" ]] && pacman -Sc --noconfirm
fi
    ...
    if command -v flatpak &>/dev/null; then
        ...
        for name in "${!FLATS[@]}"; do
            if ! flatpak list | grep -qi "${FLATS[$name]}"; then
                flatpak install -y flathub "${FLATS[$name]}" && log "[✔] $name installed"
            else
        ...
        if ! command -v trayscale &>/dev/null && ! flatpak list | grep -qi "dev.deedles.Trayscale"; then
            if ! flatpak install -y flathub dev.deedles.Trayscale; then
                if command -v go &>/dev/null; then
                    su - "$REAL_USER" -c 'go install deedles.dev/trayscale/cmd/trayscale@latest'
            ...
            if [ -d "$TMP_DIR/updates" ]; then
                ...
                if [ ${#SCRIPTS[@]} -gt 0 ]; then
                    ...
                    if [[ -n "$SEL" && "$SEL" != "SKIP" ]]; then
                        cp "$TMP_DIR/updates/$SEL" /usr/local/bin/update
            ...
            if [ ${#ROOT_SCRIPTS[@]} -gt 0 ]; then
                ...
                if [[ "$SEL" == "ALL" ]]; then
                    for f in "$TMP_DIR"/*; do
                        if [ -f "$f" ]; then
                            base=$(basename "$f")
                            cp "$f" "/usr/local/bin/${base%.*}"
                            chmod 755 "/usr/local/bin/${base%.*}"
                        fi
                    done
                    log "[✔] Installed ALL tool scripts"
                elif [[ -n "$SEL" && "$SEL" != "SKIP" ]]; then
                    cp "$TMP_DIR/$SEL" "/usr/local/bin/${SEL%.*}"
                    chmod 755 "/usr/local/bin/${SEL%.*}"
...
if [[ "$CHOICES" == *"TERM"* ]]; then
    ...
    [ -f "$BASHRC_PATH" ] && cp "$BASHRC_PATH" "$BASHRC_PATH.bak" && chown "$REAL_USER" "$BASHRC_PATH.bak"
...
EOF
    chmod +x "$KONSOLE_SH"
    su - "$REAL_USER" -c "bash $KONSOLE_SH"
    rm -f "$KONSOLE_SH"
...
EOF
    ...
    su - "$REAL_USER" -c "bash $ALACRITTY_SH"
    rm -f "$ALACRITTY_SH"
...
if [[ "$CHOICES" == *"DESKTOP"* ]]; then
    if [[ "$DE" == "kde" ]]; then
        log "[+] Configuring KDE Plasma Wallpaper and Dock..."
        
        # 1. Fetch wallpapers from repository
        WALL_TMP=$(mktemp -d)
        ...
        if git clone --depth 1 "https://github.com/77777kaz77777/wallpapers.git" "$WALL_TMP"; then
            ...
            if [ ${#WALL_FILES[@]} -gt 0 ]; then
                ...
                if [ -n "$SELECTED_WALL" ]; then
                    ...
                    su - "$REAL_USER" -c "$DBUS_ENV plasma-apply-wallpaperimage \"$TARGET_WALL_PATH\"" && log "[✔] Applied wallpaper: $SELECTED_WALL"
                fi
            else
        ...
        # 2. Setup Dock Taskbar pinned applications in exact order: Brave, Firefox, Sublime, Alacritty, Dolphin
        USER_UID=$(id -u "$REAL_USER")
        DBUS_ENV="export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/$USER_UID/bus;"

        # Find desktop file for Brave (brave-browser or brave-origin)
        BRAVE_DESKTOP="applications:brave-browser.desktop"
        [ -f /usr/share/applications/brave-origin.desktop ] && BRAVE_DESKTOP="applications:brave-origin.desktop"

        PLASMA_JS="/tmp/plasma_dock_setup.js"
        cat << EOF > "$PLASMA_JS"
var dock = new Panel();
...
EOF
        su - "$REAL_USER" -c "$DBUS_ENV qdbus org.kde.plasmashell /PlasmaShell org.kde.PlasmaShell.evaluateScript \"\$(cat $PLASMA_JS)\""
        rm -f "$PLASMA_JS"
        
        log "[✔] KDE Plasma dock configured with ordered pinned launchers."
    else
